...
namespace end
{
	...
	void dev_app_t::update()
	{
		...
		//Emitter
		SortedInit(Emitter, moveMatrix[3].xyz, float4(RAND_FLOAT(0, 1), RAND_FLOAT(0, 1), RAND_FLOAT(0, 1), 1));

		//FreeInit(Pool, redEmitter, { 7, 0, 0 }, { 1, 0, 0, 1 });
		//FreeInit(Pool, blueEmitter, { -7, 0, 0 }, { 0, 0, 1, 1 });
		//FreeInit(Pool, greenEmitter, { 7, 0, -7 }, { 0, 1, 0, 1 });
		//FreeInit(Pool, yellowEmitter, { -7, 0, -7 }, { 1, 1, 0, 1 });
	}
}
