...
bool TryToGetResources(Drinker* currentDrinker)
{
	...
	//deconstruct and add it into a loop
	///////////////////////////////////////////////////////////////////////////////////
...
void DrinkerThreadEntrypoint(Drinker* currentDrinker)
{
	...
	////////////////////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> lk(drinkerPool->startingGunMutex);
	drinkerPool->drinkerCountMutex.lock();
	drinkerPool->drinkerCount++;
	drinkerPool->drinkerCountCondition.notify_all();
	drinkerPool->drinkerCountMutex.unlock();
	drinkerPool->startingGunCondition.wait(lk);
	lk.unlock();
...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < drinkerCount; i++)
	{
		std::thread temp(DrinkerThreadEntrypoint, &poolOfDrinkers.drinkers[i]);
		temp.detach();
	}
	...
	std::unique_lock<std::mutex> DrinkerUniqueLock(poolOfDrinkers.drinkerCountMutex);
	poolOfDrinkers.drinkerCountCondition.wait(DrinkerUniqueLock, [&] {
		if (poolOfDrinkers.drinkerCount == poolOfDrinkers.totalDrinkers)
		{
			return true;
		}
		else
	...
	//   must wait for changes in the pool of drinkers to avoid burning CPU cycles.
	///////////////////////////////////////////////////////////////////////////////////
	DrinkerUniqueLock.lock();
	poolOfDrinkers.drinkerCountCondition.wait(DrinkerUniqueLock, [&]
		{
			if (poolOfDrinkers.drinkerCount == 0)
			{
				return true;
			}
			else
