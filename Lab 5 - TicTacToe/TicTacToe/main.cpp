...
struct PlayerPool
{
	...
	int* PlayerCount;
	//2 Mutexs
	std::mutex totalPlayerCountMutex;
	std::mutex GunMutex;
	//conditional variables
	std::condition_variable totalPlayerCountCondition;
	std::condition_variable ReadyCountCondition;
...
void PlayerThreadEntrypoint(Player* currentPlayer)
{
	...
	//
	///////////////////////////////////////////////////////////////////////////////////
	std::unique_lock<std::mutex> lk(currentPlayer->playerPool->GunMutex);
	{
		currentPlayer->playerPool->totalPlayerCountMutex.lock();
		currentPlayer->playerPool->ReadyCount++;
		currentPlayer->playerPool->ReadyCountCondition.notify_all();
		currentPlayer->playerPool->totalPlayerCountMutex.unlock();
		currentPlayer->playerPool->totalPlayerCountCondition.wait(lk);
	}
	...
	///////////////////////////////////////////////////////////////////////////////////
	currentPlayer->playerPool->totalPlayerCountMutex.lock();
	(*currentPlayer->playerPool->PlayerCount)--;
	currentPlayer->playerPool->ReadyCountCondition.notify_all();
	currentPlayer->playerPool->totalPlayerCountMutex.unlock();
}
///////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char** argv)
{
	...
	poolOfPlayers.startingGunFlag = false;
	//poolOfPlayers.GunMutex = gunMutex;
	//poolOfPlayers.totalPlayerCountMutex = CountMutex;
	poolOfPlayers.PlayerCount = &totalPlayerCount;
	//poolOfPlayers.totalPlayerCountCondition = variable;
	poolOfPlayers.ReadyCount = 0;
	...
	///////////////////////////////////////////////////////////////////////////////////
	for (size_t i = 0; i < totalPlayerCount; i++)
	{
		std::thread temp(PlayerThreadEntrypoint, &perPlayerData[i]);
		temp.detach();
	}
	...
	std::unique_lock<std::mutex> totalPlayerCountUniqueLock(poolOfPlayers.totalPlayerCountMutex);
	while (poolOfPlayers.ReadyCount != totalPlayerCount)
	{
		poolOfPlayers.ReadyCountCondition.wait(totalPlayerCountUniqueLock, [&] {
			if (poolOfPlayers.ReadyCount == *poolOfPlayers.PlayerCount)
			{
				poolOfPlayers.startingGunFlag = true;
				return true;
			}
			else
				return false;
	...
	totalPlayerCountUniqueLock.unlock();
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Notify all waiting threads that they can start playing.
	///////////////////////////////////////////////////////////////////////////////////
	poolOfPlayers.GunMutex.lock();
	poolOfPlayers.totalPlayerCountCondition.notify_all();
	poolOfPlayers.GunMutex.unlock();
	///////////////////////////////////////////////////////////////////////////////////
	// TODO:: Wait for all detached player threads to complete.
	///////////////////////////////////////////////////////////////////////////////////
	totalPlayerCountUniqueLock.lock();
	//wait
	poolOfPlayers.ReadyCountCondition.wait(totalPlayerCountUniqueLock, [&]
		{
			if (*poolOfPlayers.PlayerCount == 0)
			{
				return true;
