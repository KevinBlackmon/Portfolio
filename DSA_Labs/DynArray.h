...
public:
	...
	void Reserve(size_t _newCapacity = 0) {
		// TODO: Implement this method
		/*step 1 has 4 sub conditions :
		a) newcap is 0 and cap is 0
		b) newcap is 0 and cap is not 0
		c) newcap is larger than cap
		d) newcap is smaller or equal to cap but not 0
		1) change mCapacity to the correct value
		2) make a new temporary array of the updated capacity
		3) copy all data from old array to new array using size
		4) delete old array
		5) point at new array
		*/
			if ((_newCapacity <= mCapacity) && (!(_newCapacity == 0) && !(mCapacity == 0)))
			{
				return;
			}
