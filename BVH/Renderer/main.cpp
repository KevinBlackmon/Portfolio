...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
		...
		break;
	case WM_KEYDOWN:
		dev_app.keyState[wParam] = true;
		break;
	case WM_KEYUP:
		dev_app.keyState[wParam] = false;
		break;
	case WM_MOUSEMOVE:
		dev_app.Xpos = GET_X_LPARAM(lParam);
		dev_app.Ypos = GET_Y_LPARAM(lParam);
		break;
	case WM_RBUTTONDOWN:
		dev_app.mouseState[VK_RBUTTON] = true;
		break;
	case WM_RBUTTONUP:
		dev_app.mouseState[VK_RBUTTON] = false;
		break;
	default:
...
MSG begin_main_loop()
{
	...
	dev_app.viewMatrix = &renderer.default_view.view_mat; 
	dev_app.LoadTerrain();
	renderer.addverts(dev_app.terrainVertsptr);
	renderer.addTriangles(dev_app.terrainVertsptr, dev_app.trianglesptr);
	dev_app.CreateBVH(dev_app.BVH);
	srand(time(NULL));
