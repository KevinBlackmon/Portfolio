...
namespace end
{
	...
	int classify_aabb_to_aabb(const aabb_t& aabb1, const aabb_t& aabb2)
	{
		//return -1 if not colliding 
		//return 0 if colliding
	...
	void dev_app_t::LoadTerrain()
	{
		std::vector<float3> pos, nrm;
		std::vector<float2> UV;
		std::ifstream file;
		file.open("../Terrain.bin", std::ios_base::binary);
		int numverts = 0;
		if (file.is_open())
		{
			...
			file.read((char*)pos.data(), sizeof(float3) * numverts);
			file.read((char*)nrm.data(), sizeof(float3) * numverts);
			file.read((char*)UV.data(), sizeof(float2) * numverts);

			file.close();
		}
		for (size_t i = 0; i < 23814; i++)
		{
			Terrian_vertex temp;

			temp.pos = pos[i];
			temp.pos.y -= 15.6862745;
			temp.nrm = nrm[i];
			temp.uv = UV[i];

			terrainVertsptr.push_back(temp);
		}
	}

	void dev_app_t::CreateBVH(bounding_volume_hierarchy_t& bvh2)
	{
		std::vector<aabb_t>AABBs;
		for (size_t i = 0; i < trianglesptr.size(); i++)
		{
			AABBs.push_back(bvh2.buildAABB(trianglesptr[i]));
		}
		std::random_device rd;
		std::mt19937 g(rd());
		std::shuffle(AABBs.begin(), AABBs.end(), rd);

		for (size_t i = 0; i < AABBs.size(); i++)
		{
			bvh2.insert(AABBs[i], i);
		}

	}

	void Transverse(bounding_volume_hierarchy_t bvh,bvh_node_t node, std::vector<aabb_t>& branches)
	{
		if (aabb_to_frustum(node.aabb(), frustum))
		{
			branches.push_back(node.aabb());
			if (node.is_branch())
			{
				Transverse(bvh, bvh.bvh[node.left()], branches);
				Transverse(bvh, bvh.bvh[node.right()], branches);
			}
		}
	}
	...
	{
		std::vector<aabb_t> branches;
		end::debug_renderer::clear_lines();
		delta_time = calc_delta_time();

		calculate_frustum(frustum, moveMatrix);
		Transverse(BVH, BVH.bvh[0], branches);
		for (size_t i = 0; i < branches.size(); i++)
		{
			end::debug_renderer::Drawaabb(branches[i], float4(1,0,0,1));
		}
