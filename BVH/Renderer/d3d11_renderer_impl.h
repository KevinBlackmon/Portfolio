...
namespace end
{
	...
	public:
		...
		std::vector<end::Terrian_vertex> terrainVerts;
		...
		*/
		...
		void AddVerts(std::vector<end::Terrian_vertex> terrainVertsptr)
		{
			for (size_t i = 0; i < terrainVertsptr.size(); i++)
			{
				terrainVerts.push_back(terrainVertsptr[i]);
			}
			create_terrain_buffer(terrainVerts.data(), sizeof(end::Terrian_vertex) * terrainVerts.size());
		}
		...
		void draw_view(view_t& view)
		{
			...
			/*for (const auto& node : BVH.bvh)
			{
				end::debug_renderer::Drawaabb(node.aabb(), float4(1, 0, 0, 1));
			}*/
			...
			if (end::debug_renderer::get_line_vert_count() > 0)
			{
				unsigned int stride = sizeof(Terrian_vertex);
				unsigned int offset = 0;
				context->IASetVertexBuffers(0, 1, &vertex_buffer[VERTEX_BUFFER::TERRAIN], &stride, &offset);
				context->VSSetShader(vertex_shader[VERTEX_SHADER::TERRAIN], nullptr, 0);
				context->PSSetShader(pixel_shader[PIXEL_SHADER::TERRAIN], nullptr, 0);
				context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
				context->VSSetConstantBuffers(0, 1, &constant_buffer[CONSTANT_BUFFER::MVP]);
				context->IASetInputLayout(input_layout[INPUT_LAYOUT::TERRAIN]);
				context->UpdateSubresource(constant_buffer[CONSTANT_BUFFER::MVP], 0, nullptr, &mvp, 0, 0);

				context->Draw(23814, 0);
			}
		...
		void Createtriangles(std::vector<end::Terrian_vertex> terrainVertsptr, std::vector<end::Triangle>& triangles)
		{
			for (size_t i = 0; i < 23811; i += 3)
			{
				Triangle temp;
				temp.alpha = terrainVertsptr[i].pos;
				temp.beta = terrainVertsptr[i + 1].pos;
				temp.gamma = terrainVertsptr[i + 2].pos;
		...
		void setup_rasterizer()
		{
			...
			hr = device->CreateRasterizerState(&rasterDesc, &rasterState[STATE_RASTERIZER::WIREFRAME]);
		...
		void create_terrain_shaders()
		{
			{
				binary_blob_t vs_terrain_blob = load_binary_blob("TerrainVertex.cso");
				binary_blob_t ps_terrain_blob = load_binary_blob("TerrianPixel.cso");

				HRESULT hr = device->CreateVertexShader(vs_terrain_blob.data(), vs_terrain_blob.size(), NULL, &vertex_shader[VERTEX_SHADER::TERRAIN]);

				assert(!FAILED(hr));

				hr = device->CreatePixelShader(ps_terrain_blob.data(), ps_terrain_blob.size(), NULL, &pixel_shader[PIXEL_SHADER::TERRAIN]);
				...
				device->CreateInputLayout(desc, ARRAYSIZE(desc), vs_terrain_blob.data(), vs_terrain_blob.size(), &input_layout[INPUT_LAYOUT::TERRAIN]);
			}
		}
		...
		//Creating terrain buffer
		void create_terrain_buffer(void* ptr, unsigned int sizeBytes)
		{
			D3D11_BUFFER_DESC debug_desc = {};
			debug_desc.ByteWidth = sizeBytes;
			debug_desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
			debug_desc.Usage = D3D11_USAGE_DEFAULT;
			debug_desc.CPUAccessFlags = 0;
			debug_desc.MiscFlags = 0;
			...
			D3D11_SUBRESOURCE_DATA debug_srd = {};
			debug_srd.pSysMem = ptr;
			debug_srd.SysMemPitch = 0;
			debug_srd.SysMemSlicePitch = 0;

			device->CreateBuffer(&debug_desc, &debug_srd, &vertex_buffer[VERTEX_BUFFER::TERRAIN]);
		}
	};
}